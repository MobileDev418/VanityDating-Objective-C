...
-(instancetype) initWithButtons:(NSArray*) buttonsArray direction:(MGSwipeDirection) direction differentWidth:(BOOL) differentWidth
{
    ...
    for (UIView * button in buttonsArray) {
        containerWidth += button.bounds.size.height;
        maxSize.width = MAX(maxSize.width, button.bounds.size.height);
...
-(void) resetButtons
{
    CGFloat offsetX = 0;
    for (UIView * button in _buttons) {
        button.frame = CGRectMake(offsetX, 0, button.bounds.size.height, self.bounds.size.height);
        button.autoresizingMask = UIViewAutoresizingFlexibleHeight;
...
-(void) layoutExpansion: (CGFloat) offset
{
    ...
    _container.frame = CGRectMake(_fromLeft ? 0: self.bounds.size.height - offset, 0, offset, self.bounds.size.height);
    ...
    if (!_expandedButton) {
        ...
        if (!_fromLeft) { //Fix expansion animation for right buttons
            for (UIView * button in _buttons) {
                ...
                frame.origin.x += _container.bounds.size.height - previusRect.size.height;
                button.frame = frame;
            }
        }
        ...
        [UIView animateWithDuration: duration animations:^{
            ...
            if (_fromLeft) {
                _expandedButton.frame = CGRectMake(_container.bounds.size.width - _expandedButton.bounds.size.width, 0, _expandedButton.bounds.size.height, _expandedButton.bounds.size.height);
                _expandedButton.autoresizingMask|= UIViewAutoresizingFlexibleLeftMargin;
            }
            else {
                _expandedButton.frame = CGRectMake(0, 0, _expandedButton.bounds.size.height, _expandedButton.bounds.size.height);
